...
def _website_conversation(client, tenant):
    ...
    convs = client.get(f"/api/conversations?shop_id={shop}", headers=h).json()["items"]
    return convs[0]
...
def test_agent_reply_delivered_and_records_sender(client, tenant):
    ...
    conv2 = next(c for c in client.get(f"/api/conversations?shop_id={tenant['shop_id']}", headers=h).json()["items"] if c["id"] == conv["id"])
