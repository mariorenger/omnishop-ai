...
import { Admin, Billing, Channels, Help, Inbox, Knowledge, Members, Overview, Products, Settings } from "./pages";
import {
  Bot, LayoutDashboard, MessagesSquare, Package, BookOpen, Plug, Users,
...
const NAV: [string, any, string][] = [
  ...
  ["channels", Plug, "Kênh kết nối"],
  ["members", Users, "Thành viên"],
  ["billing", CreditCard, "Thanh toán"],
  ["settings", Cog, "Cài đặt"],
  ["help", LifeBuoy, "Hướng dẫn"],
];
const NEEDS_SHOP = ["overview", "inbox", "products", "knowledge", "channels"];
...
export default function App() {
  ...
  const nav = user.is_platform_admin ? [...NAV, ["admin", Shield, "Quản trị hệ thống"] as [string, any, string]] : NAV;
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 md:grid-cols-[240px_1fr]">
      ...
      <div className="flex flex-col min-w-0">
        ...
        <main className="p-5 md:p-6 max-w-[1000px] w-full">
...
function ShopSetup({ onCreated }: { onCreated: () => void }) {
  ...
  return (
    <Card className="max-w-md">
      ...
      <p className="text-[13px] text-muted mb-3 font-normal">Bạn cần một cửa hàng để thêm sản phẩm, kiến thức và bật trợ lý AI.</p>
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      <div className="hidden lg:flex flex-col justify-between p-12 bg-gradient-to-br from-panel to-bg border-r border-line">
        ...
        <div>
          <h1 className="text-4xl font-extrabold leading-tight tracking-tight">Trợ lý AI đa kênh<br />cho cửa hàng của bạn</h1>
          <p className="text-muted mt-4 max-w-md font-normal">Kết nối kênh bán, nhập sản phẩm và kiến thức, để trợ lý AI trả lời khách hàng tự động và chuyển cho nhân viên khi cần.</p>
          <div className="flex gap-4 mt-8 text-sm text-muted font-normal">
            <span className="flex items-center gap-2"><Sparkles className="w-4 h-4 text-accent" /> Hiểu ngữ cảnh sản phẩm</span>
            <span className="flex items-center gap-2"><Plug className="w-4 h-4 text-accent" /> Tự chọn nhà cung cấp AI</span>
          </div>
        </div>
